...
public class DummyL7 extends CSTA_Layer_7_Common {
	...
	public void acceptLogins() {
		...
		while (!server.isLoggedIn() && isRunFlag()) {
			...
			while (dl5.sizeOfWorkIN() > 0) {
				System.out
						.println("New messages in layer 5's IN worklist to be processed");
				str = dl5.getWorkIN();
				TestChris(str);
				// str = dl5.strip(str) ;
				// TestChris(str) ;
				...
				if (isExpectAARQ()) {
					...
					System.out
							.println(Integer.toHexString((int) str.charAt(0)));
	...
	public void setExpectAARQ(boolean expectAARQ) {
		System.out
				.println("Setting AARQ expectations to false, we've started a log in procedure");
	...
	private void sendAARQRESPONSE() {
		char[] tmp = { 0x61, 0x48, 0xa1, 0x7, 0x6, 0x5, 0x2b, 0xc, 0x0, 0x81,
				0x5a, 0xa2, 0x3, 0x2, 0x1, 0x0, 0xa3, 0x5, 0xa1, 0x3, 0x2, 0x1,
				0x0, 0x88, 0x2, 0x6, 0x80, 0xaa, 0x21, 0xa2, 0x1f, 0xa0, 0x1d,
				0xa1, 0x1b, 0x4, 0x10, 0x48, 0x45, 0x36, 0x34, 0x35, 0x50,
				0x2e, 0x30, 0x30, 0x2e, 0x30, 0x32, 0x33, 0x3a, 0x36, 0x30,
				0x4, 0x7, 0x30, 0x32, 0x33, 0x2e, 0x30, 0x30, 0x30, 0xbe, 0xa,
				0x28, 0x8, 0xa0, 0x6, 0xa0, 0x4, 0x3, 0x2, 0x4, 0x10 };
	...
	private void sendROSEINVOKE() {
		char[] tmp = { 0xa1, 0xc, 0x2, 0x1, 0x1, 0x2, 0x2, 0x0, 0xd3, 0x30,
				0x3, 0xa, 0x1, 0x2 };
		String s = new String(tmp);
		dl5.addWorkOUT(new StringBuffer(s));
		synchronized (dl5) {
		...
		while (curInStr.length() != 0) {
			...
			if (curInStr.charAt(0) == 0xA1 && untouched == true) {
				...
				if (curInStr.charAt(2) == 0x15) {// CSTA EVENT
					...
					System.out
							.println("New SMART Client Connected to CSTA Server - needs extra capabilities");
					// SEND response to this request

				} else if (curInStr.charAt(2) == 0x0a) {// make call
					System.out.println("Make call");
					server.addCSTARequest(new Make_Call_Request(
							wholeReceivedLayer7String));
					// create make call request, put into server's workpool,
					// interrupt thread.log,report.done.
				...
				} else if (curInStr.charAt(2) == 0x32) {// single step transfer
														// call
				...
				System.out
						.println("Bad comms. Probably missing bytes - Trashing this string");
				curInStr = new StringBuffer();
			}
		}
		return true;
