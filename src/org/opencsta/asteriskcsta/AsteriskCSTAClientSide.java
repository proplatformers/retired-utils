	...
	private void checkBuffer(){
		if( chris.length() > 5 ){
			...
			if( isBufferResetableAndEven(chris) ){
				System.out.println(this.getClass().getName() + " -> " + "Incoming Buffer is even and being reset");
                setComplete(true) ;
			}
			else if( isBufferStillReading(chris) ){
                System.out.println(this.getClass().getName() + " -> " + "Buffer is still reading") ;
...
//				System.out.println("The length of chris full message should be (dec): " + Integer.toString( chris.charAt(2)) ) ;
                ...
                System.out.println(this.getClass().getName() + " -> " + "Buffer is holding more than one message");
...
//				TestChris(chris) ;
	...
	private boolean isBufferStillReading(StringBuffer sb){
		String bufLength = Integer.toString(chris.length() ) ;
                ...
                System.out.println(this.getClass().getName() + " -> " + "Buffer status -> Intended Length: " + intendedLength + " | Current Length: " + bufLength ) ;
		if( chris.length() < (((int)chris.charAt(2)) + 3) ){
                    System.out.println(this.getClass().getName() + " -> " + "Buffer is still reading") ;
                    return true ;
		}
                System.out.println(this.getClass().getName() + " -> " + "Buffer is complete, now ready for clearing") ;
		return false ;
	}
